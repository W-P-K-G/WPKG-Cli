...
public class CryptoManagerGPU {
    ...
    public enum CryptoCurrencies {
        ETC(new String[] { "etc.2miners.com:1010" }), ETH(new String[] {}),
    ...
    public void refreshWallets(JComboBox<String> walletComboBox) {
        JSONParser.walletJSON[] walletJSONS = JSONParser
                .getWallet(Tools.readStringFromURL(Globals.jsonURL + "Wallets.json"));
    ...
    private void runAction() {
        try {
            ...
            errorHandler.check(Client.sendCommand("startminer " + algorithmField.getSelectedItem() + " "
                    + poolComboBox.getSelectedItem() + " " + Objects.requireNonNull(walletComboBox.getSelectedItem())
                            .toString().replace("workername", workerField.getText())));
